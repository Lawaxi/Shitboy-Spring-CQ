...
public class WeiboSender extends Sender {
    ...
    public WeiboSender(CoolQ bot, long group, HashMap<String, Long> endTime) {
        super(bot, group);
    ...
    public void run() {
        ...
        try {
            //超话
            List<String> superChatSubscribe = Shitboy.INSTANCE.getProperties().weibo_superTopic_subscribe.get(group);
        ...
        try {
            //个人
            List<Long> userSubscribe = Shitboy.INSTANCE.getProperties().weibo_user_subscribe.get(group);
        ...
        for (messageWithTime m : ms) {
            bot.sendGroupMsg(group, toNotification(m.message), false);
        }
    }

    private String parseUserBlog(JSONObject b) throws IOException {
        ...
        //转发
            ...
            o += "------\n\n" + (retweet_from == null ? "" : "@" + retweet_from + "：") + parseUserBlog(retweet);//叠呗
        }
    ...
    public String getImgRes(String resLoc) {
        return super.getImgRes(resLoc);
        //.setReadTimeout(20000).header("Referer", "https://weibo.com/").execute().bodyStream();
    }
    ...
    private class messageWithTime {
        ...
        private messageWithTime(String message, long time) {
